...
以ajax请求为例：
 ...
 ```
  ...
  4. 创建：status 三个状态：pending/fulfilled/rejected
  5. 创建：resolveCallbacks rejectCallbacks 存放成功或失败的回调
  6. 执行；executor,发送ajax请求
  ...
  9. 执行完主线程所有宏任务，然后清空微任务，此时执行onfulfilled或onrejected

# 实现过程
参考[promises/A+](https://promisesaplus.com/)文档
```javascript
