...
如之前所述，在MongoDB中，写操作在单个文档上保证原子性。即便MongoDB从4.0开始支持多事务，但在很多情况下，采用非标准的数据模型（子文档），仍然是最佳选择。因此对于那些必须一起更新的字段，可以考虑将其嵌入一个文档中，以保证原子性操作。
...
通常来说，采用非标准的数据模型（子文档）是最佳选择。但是，在某些情况下，将关联的信息分开存储到不同的集合或者数据库中也是合理的。
